...
export default function ApprovePurchase(props) {
  ...
  const fetchUsers = () => {
    axios
      .post('http://localhost:5000/pendingPurchase')
      .then((res) => {
        console.log("approvePurchase ",res.data);
  ...
  const handleApprove = (purchaseId,userId) => {
    ...
    axios.post('http://localhost:5000/pendingPurchase1',{purchaseId,userId,msg})
    .then((res)=>{
        console.log("approve purchase pendingpurchase1 ",res);
    })
    .catch(err=>{
        console.log(err);
    })
