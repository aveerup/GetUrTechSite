...
const [msg,setMsg]=useState('');
  ...
  function fetchData() {
    axios.post('http://localhost:5000/showdetails', { id, userId })
      .then(res => {
        console.log(res);
  ...
  function fetchComments(product_id) {
    axios.get(`http://localhost:5000/getComments/${product_id}`)
      .then((res) => {
  ...
  const checkLoginStatus=(id)=>{
    ...
    { 
      ...
      const u_id=props.user;
      axios.post('http://localhost:5000/cart',{id,u_id})
      .then(res => {
  ...
  const addComment = () => {
    ...
    axios.post('http://localhost:5000/addComment', newCommentObject)
      .then((res) => {
        fetchComments(id);
        setNewComment(''); 
        setMsgController(4);
  ...
  const addReply = (commentId) => {
    ...
    axios.post('http://localhost:5000/addReply', newReplyObject)
      .then((res) => {
  ...
  const handleSubmit = (e) => {
    ...
    e.preventDefault();
    axios.post('http://localhost:5000/productReview', {product_id,userId,rating,review})
      .then((res) => {       
    ...
    <div className="details">                  
            ...
            <img src = {require('../image/'+productInfo.img_url)} alt = "ProductImage" /> 
                ...
                
              {
                ...
                {userRole==='admin' ?( <button type="button" className="btn btn-outline-success" onClick={()=>{navigate(`/updateProduct?itemId=${productInfo.id}`)}}>Update Info</button>)
                :( productInfo.base_price > 0 ? (
                  <div className='buttons1'>
                  <button type="button" className="btn btn-outline-success" style={{marginRight:'30px'}} onClick={()=>{checkLoginStatus(productInfo.id)}}>Add To Cart</button>
                  
                  <button type="button" className="btn btn-outline-success" onClick={()=>{addtocomp(productInfo)}}>Add To Compare</button>
                  </div>
                ...
                {ShowAlert && (
                  msgController === 1 ? (
                    ...
                    <CustomAlert
                    message="User must use the product for at least 3 days before writing a review. "
                    onClose={handleCloseAlert}
                    type="success"
                  />
                  ):(
